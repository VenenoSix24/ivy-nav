...
describe("trimTransform", () => {
  ...
  it("scales a centred inset so the artwork fills the box", () => {
    // 中心在 0.45，放大两倍之后还要向右下挪 0.05 才回到正中
    ...
    expect(box.y0).toBeCloseTo(0);
    expect(box.y1).toBeCloseTo(1);
  });
  ...
  it("shifts the artwork when the transparent margin is lopsided", () => {
    ...
    // 内容 0.5 见方、中心在 0.35：放大两倍后要挪 0.15 才回到正中
  ...
  it("never scales past 2×", () => {
    ...
    expect(trimTransform(tiny, 1)?.scale).toBe(2);
  });
});
