...
- **底板**：图标底下那层玻璃底与描边（圆角 8px，三种布局一致）。应用类图标（比如羊毛图标库那种
  ...
  图标因此不像印在卡面上。底板关掉时方块没有面，影子改跟着图案走（`drop-shadow` 挂在图形
  容器上，所以 emoji、首字母托底也都有）—— 两种情况都只有一层影子，不会出现「盒子里套盒子」。
- **Emoji** 不套底板（选择器里那个开关对它不出现），按行内盒居中摆着 ——
  emoji 自己就是一块彩色图案，各家字体把图案摆在字框里的位置又各不相同，垫一层只会更显歪。图形大小按盒子算
