...
- **底板**：图标底下那层描边与玻璃底。应用类图标（比如羊毛图标库那种自带圆角外形的）套上
  底板就成了大圆套小圆，关掉它格子尺寸不变，只是不再画那层底。图形大小按盒子算
  （容器查询单位 `cqh`），所以关掉底板后图形会自己涨到接近满格，不会显得比原来小一圈。
...
**先预览、后写入**：选好文件会先给出分类与标签的清单（哪些新建、哪些重名复用、哪些行读不了），
确认之后才落库。导入只新增与更新，不删除任何现有内容。

重复网址的处理有两个选择：

- **跳过**（默认）：库里已经有同一个网址就原样不动；
- **覆盖**：用书签里的标题、描述与分类更新那一条，标签是并进去而不是换掉 —— 用户自己在这条上打的标签不会丢。

同一份文件里出现两次的网址只留第一次。图标不会在导入时抓取：几百条会把外网拉爆，交给首页按需缓存。
