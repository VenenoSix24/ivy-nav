...
- **网站图标**要显式获取：图标选择器「网站图标」页里点「获取图标」才去抓，抓到显示图标、
  抓不到直接说明并给出替代做法；抓取结果按站点缓存在服务器上。取图顺序是网页里声明的
  `<link rel="icon">` → PWA manifest 里声明的图标 → `/favicon.ico` → 第三方图标服务
  （favicon.im、icon.horse，可用 `FAVICON_FALLBACK_SOURCES=false` 关掉）。服务自己那张
  「查不到」占位图会被认出来丢掉，不会拿它冒充站点图标。
