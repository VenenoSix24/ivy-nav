...
- **底板**：图标底下那层玻璃底与描边。应用类图标（比如羊毛图标库那种自带圆角外形的）套上
  底板仍是大圆套小圆，那就按条目关掉它：格子尺寸不变，只是不再画那层底。
  浅色下这个方块自己带一层很浅的投影（深色下不加），图标因此不像印在卡面上。
  底板关掉时方块没有面，影子改跟着图案走（`drop-shadow`，透明区域不带影子）——
  两种情况都只有一层影子，不会出现「盒子里套盒子」。图形大小按盒子算
...
| 选法         | 做法                                                                                  |
| ------------ | ------------------------------------------------------------------------------------- |
| 原样（默认） | 整张图按比例放进去，四周留白照旧                                                      |
| 自动裁边     | 在浏览器里把图标画进一张 48×48 的画布，数出不透明像素的范围，裁掉透明边让图案自己填满 |
| 裁剪铺满     | 放大到铺满底板，超出的部分裁掉                                                        |
| 拉伸铺满     | 拉到和底板一样大，不是正方形会被压扁                                                  |

自动裁边的换算是一个纯函数（`lib/icons/fit.ts` 的 `trimTransform`，有单测盯着），只在
「图案占画布 34%–94%」这一档才动手：图案已经顶格就不必裁，小得可疑（多半是量错了）就放过，
