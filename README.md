...
- **底板**：图标底下那层玻璃底与描边（圆角 8px，三种布局一致）。应用类图标（比如羊毛图标库那种
  自带圆角外形的）套上底板仍是大圆套小圆，那就按条目关掉它：格子尺寸不变，只是不再画那层底。
  浅色下这个方块自己带一层分层柔和阴影（模糊 2/4/8、透明度 7%/5%/3%，深色下不加），
  图标因此不像印在卡面上。底板关掉时方块没有面，影子改跟着图案走（`drop-shadow`）——
  两种情况都只有一层影子，不会出现「盒子里套盒子」。
- **Emoji** 会先量一遍墨迹再挪到正中、缩到统一大小（各家 emoji 字体把图案摆得高低不一，
  Apple 的键盘就明显偏下），换算与图标裁边同源（`lib/icons/emoji-ink.ts`）。图形大小按盒子算
...
| 自动裁边     | 在浏览器里把图标画进一张 48×48 的画布，数出不透明像素的范围，裁掉透明边让图案顶到板边 |
...
放大倍数额外封顶 2 倍 —— 再大位图就开始糊了。图片都走本站代理，同源，所以画布读得出来；
