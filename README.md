...
- **底板**：图标底下那层玻璃底与描边（圆角按 Apple 应用图标的比例给，22%，随格子大小走）。
  应用类图标（比如羊毛图标库那种自带圆角外形的）套上底板仍是大圆套小圆，那就按条目关掉它：
  格子尺寸不变，只是不再画那层底。
  浅色下**无论有没有底板，这个方框都有**一层分层柔和阴影（模糊 2/4/8、透明度 7%/5%/3%，
  深色下不加）—— 影子挂在方框上而不是图案上，用途就是让「图标这一块」与卡面分开；
  没有底板时方框没有面，这层影子就是唯一的那一层。
